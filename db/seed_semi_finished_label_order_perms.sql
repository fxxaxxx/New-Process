-- 为现有账号及权限主体增量补齐半成品标签单权限，不改写已有设置。
-- 产品价格仍由本菜单的“单价”权限控制；默认种子不授予该可选权限。
SET XACT_ABORT ON;
...
BEGIN TRY
    ...
    EXEC @锁结果 = sys.sp_getapplock
        @Resource = N'db.seed_semi_finished_label_order_perms',
        @LockMode = N'Exclusive',
        @LockOwner = N'Transaction',
        @LockTimeout = 60000;

    IF @锁结果 < 0
        THROW 51001, N'无法取得半成品标签单权限种子锁', 1;
    ...
    IF @跳过账号数 > 0
        PRINT CONCAT(N'半成品标签单权限种子：跳过 ', @跳过账号数,
...
WHEN NOT MATCHED BY TARGET THEN
    ...
    VALUES ([来源].[用户], [来源].[名称], N'半成品标签单', 1, 1, 1, 1, 1, 1);
...
IF EXISTS (
    ...
    THROW 51002, N'半成品标签单权限存在重复主体', 1;

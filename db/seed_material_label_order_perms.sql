-- 为现有账号及权限主体增量补齐 来料标签单 / 采购物料设置 权限，不改写已有设置。
SET XACT_ABORT ON;
...
BEGIN TRY
    ...
    EXEC @锁结果 = sys.sp_getapplock
        @Resource = N'db.seed_material_label_order_perms',
        @LockMode = N'Exclusive',
        @LockOwner = N'Transaction',
        @LockTimeout = 60000;

    IF @锁结果 < 0
        THROW 51001, N'无法取得来料标签单权限种子锁', 1;
    ...
    IF @跳过账号数 > 0
        PRINT CONCAT(N'来料标签单权限种子：跳过 ', @跳过账号数,
...
WHEN NOT MATCHED BY TARGET THEN
    ...
    VALUES ([来源].[用户], [来源].[名称], N'来料标签单', 1, 1, 1, 1, 1, 1);
...
WHEN NOT MATCHED BY TARGET THEN
    ...
    VALUES ([来源].[用户], [来源].[名称], N'采购物料设置', 1, 1, 1);

IF EXISTS (
    SELECT 1
    FROM [userbqrpower]
    WHERE [菜单] IN (N'来料标签单', N'采购物料设置')
    GROUP BY [用户], [菜单]
    HAVING COUNT(*) > 1
)
    THROW 51002, N'来料标签单/采购物料设置权限存在重复主体', 1;

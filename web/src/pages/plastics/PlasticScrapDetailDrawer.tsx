...
export default function PlasticScrapDetailDrawer({ open, 单号, onClose }: { open: boolean; 单号?: string; onClose: () => void }) {
  const perms = usePerms();
  const priceHidden = hidePrice(perms, "塑胶报废查询");
  const [head, setHead] = useState<Record<string, unknown> | null>(null);
  const [lines, setLines] = useState<Record<string, unknown>[]>([]);
  useEffect(() => {
    if (!open || !单号) return;
    plasticDocApi("plastic-scraps").get(单号).then(d => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={900} title={`塑胶报废单 ${单号 ?? ""}`}>
      {head && (
        <Descriptions size="small" column={3} bordered style={{ marginBottom: 16 }}>
          ...
          <Descriptions.Item label="报废部门">{String(head.报废部门 ?? "")}</Descriptions.Item>
          <Descriptions.Item label="报废人">{String(head.报废人 ?? "")}</Descriptions.Item>

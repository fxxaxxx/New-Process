...
import { plasticPurchaseProgressDetailApi, type PlasticPurchaseProgressDetailRow } from "../../api/plasticPurchaseProgressDetail";
...
export default function PlasticPurchaseProgressDetailPage() {
  ...
  const [rows, setRows] = useState<PlasticPurchaseProgressDetailRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setRows(await plasticPurchaseProgressDetailApi.list({
    ...
    } catch { message.error("加载塑胶进度明细表失败"); }
    finally { setLoading(false); }
  }, [canOpen, range, supplier, keyword, done]);
  useEffect(() => { load(); }, [canOpen, range, supplier, done]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const columns: ColumnsType<PlasticPurchaseProgressDetailRow> = [
  ...
  const exportCols: ExportCol[] = [
    ...
    { title: "入仓单号", key: "入仓单号" }, { title: "入仓数量", key: "入仓数量" }, { title: "欠数", key: "欠数" },
    { title: "完成情况", key: "完成情况" }, { title: "供应商名称", key: "供应商名称" },
    { title: "审核", key: "审核", fmt: v => (v === "1" ? "已审核" : "未审核") },
  ];
  const asRecords = () => rows as unknown as Record<string, unknown>[];

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少"塑胶进度明细表·打开"权限）。</div></Card>;
  }

  return (
    <Card title="塑胶进度明细表" variant="borderless">
      <Space style={{ marginBottom: 12 }} wrap>
        ...
        <Button onClick={() => downloadCsv("塑胶进度明细表.csv", exportCols, asRecords())}>导出EXCEL</Button>
        <Button onClick={() => printTable("塑胶进度明细表", exportCols, asRecords())}>打印</Button>

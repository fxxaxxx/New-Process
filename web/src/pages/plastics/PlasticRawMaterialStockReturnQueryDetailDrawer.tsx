...
import { plasticRawMaterialStockReturnApi } from "../../api/plasticRawMaterialStockReturn";

export default function PlasticRawMaterialStockReturnQueryDetailDrawer({ open, 单号, onClose }: {
  open: boolean;
  单号?: string;
  onClose: () => void;
}) {
  ...
  useEffect(() => {
    if (!open || !单号) return;
    plasticRawMaterialStockReturnApi.get(单号).then(d => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={900} title={`原料退库表 ${单号 ?? ""}`}>
      {head && (
        <Descriptions size="small" column={3} bordered style={{ marginBottom: 16 }}>
          ...
          <Descriptions.Item label="退料部门">{String(head.部门 ?? "")}</Descriptions.Item>
          <Descriptions.Item label="退料人">{String(head.退料人 ?? "")}</Descriptions.Item>

...
export default function PlasticReturnDetailDrawer({ open, 单号, onClose }: { open: boolean; 单号?: string; onClose: () => void }) {
  const perms = usePerms();
  const priceHidden = hidePrice(perms, "塑胶退料查询");
  const [head, setHead] = useState<Record<string, unknown> | null>(null);
  const [lines, setLines] = useState<Record<string, unknown>[]>([]);
  useEffect(() => {
    if (!open || !单号) return;
    plasticDocApi("plastic-returns").get(单号).then(d => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={900} title={`塑胶退料单 ${单号 ?? ""}`}>
      {head && (
        <Descriptions size="small" column={3} bordered style={{ marginBottom: 16 }}>
          ...
          <Descriptions.Item label="退料部门">{String(head.退料部门 ?? "")}</Descriptions.Item>

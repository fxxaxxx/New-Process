...
import { plasticWarehouseReturnQueryApi, type PlasticWhReturnQueryDetailRow, type PlasticWhReturnQuerySummaryRow, type PlasticWhReturnQueryParams } from "../../api/plasticWarehouseReturnQuery";
...
import PlasticWarehouseReturnQueryDetailDrawer from "./PlasticWarehouseReturnQueryDetailDrawer";

const MENU = "塑胶退仓查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticWarehouseReturnQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticWhReturnQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticWhReturnQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticWhReturnQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticWarehouseReturnQueryApi.detail(query));
      else setSummary(await plasticWarehouseReturnQueryApi.summary(query));
    } catch { message.error("加载塑胶退仓查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticWhReturnQueryDetailRow> = [
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const name = tab === "detail" ? "塑胶退仓查询-明细" : "塑胶退仓查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“塑胶退仓查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="塑胶退仓查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 340px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticWarehouseReturnQueryDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

...
import { plasticIssueQueryApi, type PlasticIssueQueryDetailRow, type PlasticIssueQuerySummaryRow, type PlasticIssueQueryParams } from "../../api/plasticIssueQuery";
...
import PlasticIssueDetailDrawer from "./PlasticIssueDetailDrawer";

const MENU = "塑胶领料查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticIssueQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticIssueQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticIssueQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticIssueQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticIssueQueryApi.detail(query));
      else setSummary(await plasticIssueQueryApi.summary(query));
    } catch { message.error("加载塑胶领料查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticIssueQueryDetailRow> = [
    ...
    { title: "领料部门", dataIndex: "领料部门", width: 100 },
    { title: "领料人", dataIndex: "领料人", width: 90 },
    { title: "装配采购", dataIndex: "装配采购", width: 90 },
  ...
  const summaryColumns: ColumnsType<PlasticIssueQuerySummaryRow> = [
    { title: "生产单号", dataIndex: "生产单号", width: 130 },
    { title: "款号", dataIndex: "款号", width: 100 },
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const name = tab === "detail" ? "塑胶领料查询-明细" : "塑胶领料查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“塑胶领料查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="塑胶领料查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 300px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticIssueDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

...
import { plasticRawMaterialReceiptApi } from "../../api/plasticRawMaterialReceipt";
import { hidePrice } from "../../auth/permissions";
import { usePerms } from "../../auth/PermissionContext";

export default function PlasticRawMaterialReceiptQueryDetailDrawer({ open, 单号, onClose }: {
  open: boolean;
  单号?: string;
  onClose: () => void;
}) {
  const perms = usePerms();
  const priceHidden = hidePrice(perms, "原料入仓查询");
  const [head, setHead] = useState<Record<string, unknown> | null>(null);
  const [lines, setLines] = useState<Record<string, unknown>[]>([]);

  useEffect(() => {
    if (!open || !单号) return;
    plasticRawMaterialReceiptApi.get(单号).then(d => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={900} title={`原料入仓单 ${单号 ?? ""}`}>
      {head && (

...
import { plasticReturnQueryApi, type PlasticReturnQueryDetailRow, type PlasticReturnQuerySummaryRow, type PlasticReturnQueryParams } from "../../api/plasticReturnQuery";
...
import PlasticReturnDetailDrawer from "./PlasticReturnDetailDrawer";

const MENU = "塑胶退料查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticReturnQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticReturnQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticReturnQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticReturnQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticReturnQueryApi.detail(query));
      else setSummary(await plasticReturnQueryApi.summary(query));
    } catch { message.error("加载塑胶退料查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticReturnQueryDetailRow> = [
    ...
    { title: "退料部门", dataIndex: "退料部门", width: 100 },
    { title: "退料人", dataIndex: "退料人", width: 90 },
  ...
  const summaryColumns: ColumnsType<PlasticReturnQuerySummaryRow> = [
    { title: "生产单号", dataIndex: "生产单号", width: 130 },
    { title: "款号", dataIndex: "款号", width: 100 },
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const name = tab === "detail" ? "塑胶退料查询-明细" : "塑胶退料查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“塑胶退料查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="塑胶退料查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 340px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticReturnDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

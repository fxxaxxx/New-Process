...
import { plasticReceiptQueryApi, type PlasticReceiptQueryDetailRow, type PlasticReceiptQuerySummaryRow, type PlasticReceiptQueryParams } from "../../api/plasticReceiptQuery";
...
import PlasticReceiptQueryDetailDrawer from "./PlasticReceiptQueryDetailDrawer";

const MENU = "塑胶入仓查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticReceiptQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticReceiptQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticReceiptQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticReceiptQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticReceiptQueryApi.detail(query));
      else setSummary(await plasticReceiptQueryApi.summary(query));
    } catch { message.error("加载塑胶入仓查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticReceiptQueryDetailRow> = [
  ...
  const summaryColumns: ColumnsType<PlasticReceiptQuerySummaryRow> = [
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const name = tab === "detail" ? "塑胶入仓查询-明细" : "塑胶入仓查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“塑胶入仓查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="塑胶入仓查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 340px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticReceiptQueryDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

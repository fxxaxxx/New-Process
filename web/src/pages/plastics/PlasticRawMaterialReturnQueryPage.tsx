...
import {
  plasticRawMaterialReturnQueryApi,
  type PlasticRawMaterialReturnQueryDetailRow,
  type PlasticRawMaterialReturnQueryParams,
  type PlasticRawMaterialReturnQuerySummaryRow,
} from "../../api/plasticRawMaterialReturnQuery";
...
import PlasticRawMaterialReturnQueryDetailDrawer from "./PlasticRawMaterialReturnQueryDetailDrawer";
...
export default function PlasticRawMaterialReturnQueryPage() {
  ...
  const [summary, setSummary] = useState<PlasticRawMaterialReturnQuerySummaryRow[]>([]);
  const [detail, setDetail] = useState<PlasticRawMaterialReturnQueryDetailRow[]>([]);
  ...
  const query = useMemo<PlasticRawMaterialReturnQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "summary") setSummary(await plasticRawMaterialReturnQueryApi.summary(query));
      else setDetail(await plasticRawMaterialReturnQueryApi.detail(query));
    } catch {
      message.error("加载原料退仓查询失败");
    } finally {
      setLoading(false);
    }
  }, [canOpen, query, tab]);
  ...
  const summaryColumns: ColumnsType<PlasticRawMaterialReturnQuerySummaryRow> = [
    ...
    { title: "退仓数量", dataIndex: "退仓数量", width: 120, align: "right", render: fmtNum },
  ];

  const detailColumns: ColumnsType<PlasticRawMaterialReturnQueryDetailRow> = [
  ...
  const activeTitle = tab === "summary" ? "原料退仓汇总查询" : "原料退仓明细查询";
  const qtyKey = tab === "summary" ? "退仓数量" : "数量";
  const totalQty = activeRows.reduce((s, r) => s + Number((r as Record<string, unknown>)[qtyKey] ?? 0), 0);

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“原料退仓查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="原料退仓查询" variant="borderless">
      ...
      />
      <PlasticRawMaterialReturnQueryDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

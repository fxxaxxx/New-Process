...
export default function PlasticReceiptQueryDetailDrawer({ open, 单号, onClose }: { open: boolean; 单号?: string; onClose: () => void }) {
  const perms = usePerms();
  const priceHidden = hidePrice(perms, "塑胶入仓查询");
  const [head, setHead] = useState<Record<string, unknown> | null>(null);
  const [lines, setLines] = useState<Record<string, unknown>[]>([]);
  useEffect(() => {
    if (!open || !单号) return;
    plasticDocApi("plastic-receipts").get(单号).then(d => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={900} title={`塑胶入仓单 ${单号 ?? ""}`}>
      {head && (

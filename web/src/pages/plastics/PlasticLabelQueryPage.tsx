...
export default function PlasticLabelQueryPage() {
  ...
  const detailColumns: ColumnsType<PlasticLabelQueryDetailRow> = [
    ...
    { title: "电脑单号", dataIndex: "电脑单号", width: 130, render: (v: string) => <span className="erp-num">{v}</span> },
    ...
    { title: "标签数", dataIndex: "标签数", width: 70, align: "right" },
  ...
  const summaryColumns: ColumnsType<PlasticLabelQuerySummaryRow> = [
    ...
    { title: "标签数", dataIndex: "标签数", width: 90, align: "right" },
  ];
  ...
  return (
    <Card title="塑胶标签查询" variant="borderless">
      <Space style={{ marginBottom: 12 }} wrap>
        ...
        <Input.Search placeholder="电脑单号/物料编号/名称/规格" allowClear value={keyword}
        ...
        <span style={{ color: "#999" }}>提示：双击明细行可打开塑胶标签单</span>
      </Space>
      ...
      {tab === "detail"
        ? <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={detail} columns={detailColumns}
            ...
            onRow={r => ({ onDoubleClick: () => { if (r.电脑单号) navigate(`/plastic-label-orders?open=${encodeURIComponent(r.电脑单号)}`); }, style: { cursor: "pointer" } })} />

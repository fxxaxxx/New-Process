...
import { plasticProcessPurchaseQueryApi, type PlasticProcessPurchaseQueryDetailRow, type PlasticProcessPurchaseQuerySummaryRow, type PlasticProcessPurchaseQueryParams } from "../../api/plasticProcessPurchaseQuery";
...
import PlasticProcessPurchaseOrderQueryDetailDrawer from "./PlasticProcessPurchaseOrderQueryDetailDrawer";

const MENU = "加工采购查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticProcessPurchaseQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticProcessPurchaseQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticProcessPurchaseQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticProcessPurchaseQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticProcessPurchaseQueryApi.detail(query));
      else setSummary(await plasticProcessPurchaseQueryApi.summary(query));
    } catch { message.error("加载加工采购查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticProcessPurchaseQueryDetailRow> = [
  ...
  const summaryColumns: ColumnsType<PlasticProcessPurchaseQuerySummaryRow> = [
    { title: "模具编号", dataIndex: "模具编号", width: 110 },
    ...
    { title: "订购数量", dataIndex: "订购数量", width: 100, align: "right" },
    ...(priceHidden ? [] : [
      { title: "总金额", dataIndex: "总金额", width: 120, align: "right" as const, render: fix2 },
    ]),
  ];
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const cols: ExportCol[] = srcCols.map(c => {
      ...
      return {
        ...
        key,
        fmt: key === "单据日期" ? (v => String(v ?? "").slice(0, 10)) : key === "审核" ? (v => (v === "1" ? "已审核" : "未审核")) : undefined,
      };
    });
    const data = (tab === "detail" ? detail : summary) as unknown as Record<string, unknown>[];
    const name = tab === "detail" ? "加工采购查询-明细" : "加工采购查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“加工采购查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="加工采购查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 300px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticProcessPurchaseOrderQueryDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

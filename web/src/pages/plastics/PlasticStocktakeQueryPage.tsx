...
import { plasticStocktakeQueryApi, type PlasticStocktakeQueryDetailRow, type PlasticStocktakeQuerySummaryRow, type PlasticStocktakeQueryParams } from "../../api/plasticStocktakeQuery";
...
import PlasticStocktakeQueryDetailDrawer from "./PlasticStocktakeQueryDetailDrawer";

const MENU = "塑胶盘点查询";
const ALL = "__ALL__";
const thisMonth = (): [Dayjs, Dayjs] => [dayjs().startOf("month"), dayjs().endOf("month")];

export default function PlasticStocktakeQueryPage() {
  ...
  const [detail, setDetail] = useState<PlasticStocktakeQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PlasticStocktakeQuerySummaryRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [viewing, setViewing] = useState<string | undefined>(undefined);

  const query = useMemo<PlasticStocktakeQueryParams>(() => ({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await plasticStocktakeQueryApi.detail(query));
      else setSummary(await plasticStocktakeQueryApi.summary(query));
    } catch { message.error("加载塑胶盘点查询失败"); }
  ...
  const detailColumns: ColumnsType<PlasticStocktakeQueryDetailRow> = [
    ...
    { title: "系统数量", dataIndex: "系统数量", width: 90, align: "right" },
    { title: "盘点数量", dataIndex: "盘点数量", width: 90, align: "right" },
    { title: "盈亏数量", dataIndex: "盈亏数量", width: 90, align: "right" },
    ...(priceHidden ? [] : [
  ...
  const exportNow = (action: "csv" | "print") => {
    ...
    const name = tab === "detail" ? "塑胶盘点查询-明细" : "塑胶盘点查询-汇总";
    if (action === "csv") downloadCsv(`${name}.csv`, cols, data); else printTable(name, cols, data);
  };

  if (!canOpen) {
    return <Card variant="borderless"><div style={{ padding: 24, color: "#999" }}>无权访问该页面（缺少“塑胶盘点查询·打开”权限）。</div></Card>;
  }

  return (
    <Card title="塑胶盘点查询" variant="borderless">
      ...
      {tab === "detail"
        ...
        : <Table rowKey={(_, i) => String(i)} size="small" loading={loading} dataSource={summary} columns={summaryColumns}
            scroll={{ x: "max-content", y: "calc(100vh - 340px)" }} pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} />}
      <PlasticStocktakeQueryDetailDrawer open={viewing !== undefined} 单号={viewing} onClose={() => setViewing(undefined)} />
    </Card>
  );
}

...
import {
  plasticLabelOrdersApi,
  type PlasticLabelMaterialRow,
  type PlasticLabelOrderListRow,
} from "../../api/plasticLabelOrders";
...
function MaterialPickModal({ open, onPick, onClose }: {
  open: boolean;
  onPick: (row: PlasticLabelMaterialRow) => void;
  onClose: () => void;
}) {
  const [keyword, setKeyword] = useState("");
  const [rows, setRows] = useState<PlasticLabelMaterialRow[]>([]);
  ...
  const load = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const r = await plasticLabelOrdersApi.materials({ page: p, size: 50, keyword: keyword.trim() || undefined });
...
function OrderPickModal({ open, onPick, onClose }: {
  ...
  const [keyword, setKeyword] = useState("");
  const [rows, setRows] = useState<PlasticLabelOrderListRow[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const r = await plasticLabelOrdersApi.list(p, 50, keyword.trim());
      setRows(r.items); setTotal(r.total);
    } catch { message.error("加载塑胶标签单列表失败"); }
    finally { setLoading(false); }
  }, [keyword]);
  ...
  return (
    <Modal title="打开塑胶标签单" open={open} onCancel={onClose} footer={null} width={860}>
      <div style={{ marginBottom: 12 }}>
        <Input.Search
...
export default function PlasticLabelOrderPage() {
  ...
  const openOrder = useCallback(async (orderNo: string) => {
    ...
    try {
      const order = await plasticLabelOrdersApi.get(orderNo) as unknown as LabelOrder;
      if (version !== documentRequestVersion.current) return;
      applyOrder(order);
    } catch (error) {
      if (version === documentRequestVersion.current) {
        message.error(errorMessage(error, "打开塑胶标签单失败"));
      }
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (openedOrder?.电脑单号) {
        const saved = await plasticLabelOrdersApi.update(openedOrder.电脑单号, payload as never);
        if (mutation !== mutationVersion.current) return;
        applyOrder(saved as unknown as LabelOrder);
      } else {
        const saved = await plasticLabelOrdersApi.create(payload as never);
        if (mutation !== mutationVersion.current) return;
        const version = ++documentRequestVersion.current;
        const created = await plasticLabelOrdersApi.get(saved.电脑单号) as unknown as LabelOrder;
        if (mutation !== mutationVersion.current || version !== documentRequestVersion.current) return;
        applyOrder(created);
      }
      if (mutation === mutationVersion.current) message.success("塑胶标签单已保存");
    } catch (error) {
      message.error(errorMessage(error, "保存塑胶标签单失败"));
    } finally {
  ...
  const remove = async () => {
    ...
    setDeleting(true);
    try {
      await plasticLabelOrdersApi.remove(openedOrder.电脑单号);
      if (mutation !== mutationVersion.current) return;
      message.success("塑胶标签单已删除");
      reset();
    } catch (error) {
      if (mutation === mutationVersion.current) {
        message.error(errorMessage(error, "删除塑胶标签单失败"));
      }
    } finally {
      writeActive.current = false;
      setDeleting(false);
  ...
  const moveAdjacent = async (direction: "previous" | "next") => {
    ...
    const version = ++documentRequestVersion.current;
    try {
      const order = await plasticLabelOrdersApi.adjacent(openedOrder.电脑单号, direction) as unknown as LabelOrder | null;
      if (version !== documentRequestVersion.current) return;
  ...
  const changeAudit = async (reverse = false) => {
    ...
    const version = ++documentRequestVersion.current;
    try {
      if (reverse) await plasticLabelOrdersApi.reverseAudit(orderNo);
      else await plasticLabelOrdersApi.audit(orderNo);
      if (version !== documentRequestVersion.current) return;
      const order = await plasticLabelOrdersApi.get(orderNo) as unknown as LabelOrder;
      if (version !== documentRequestVersion.current) return;
      applyOrder(order);
  ...
  const pickMaterial = (material: PlasticLabelMaterialRow) => {
    if (editReadOnly || writeActive.current || auditActive.current) return;
    invalidateMutation();
    setLines(current => {
  ...
  return (
    <Card title="塑胶标签单" variant="borderless" extra={
      <Space wrap>
        ...
        <Popconfirm title="确认删除该塑胶标签单?" disabled={!openedOrder?.电脑单号 || audited || !canDelete || auditLoading !== null || saving} onConfirm={remove}>

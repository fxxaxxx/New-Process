...
export default function PurchasePaymentPage() {
  ...
  const [rows, setRows] = useState<PPHeader[]>([]);
  ...
  const load = useCallback(async () => {
    try { const r = await purchasePaymentApi.list(page, 10, keyword); setRows(r.items); setTotal(r.total); }
    catch { message.error("加载采购付款单失败"); }
  }, [page, keyword]);
  useEffect(() => { load(); }, [load]);
  ...
  const columns = [
    ...
    {
      title: "操作", key: "_op",
      render: (_: unknown, row: PPHeader) => (
        <Space>
          {row.审核 !== "1" && can(perms, MENU, "审核") && <a onClick={() => act(() => purchasePaymentApi.approve(row.单号!), "已审核")}>审核</a>}
          {row.审核 === "1" && can(perms, MENU, "反审核") && <a onClick={() => act(() => purchasePaymentApi.unapprove(row.单号!), "已反审核")}>反审核</a>}
          {row.审核 !== "1" && can(perms, MENU, "删除") && (
            <Popconfirm title="确认删除该付款单?" onConfirm={() => act(() => purchasePaymentApi.remove(row.单号!), "已删除")}><a>删除</a></Popconfirm>
          )}
        </Space>
      ),
    },
  ...
  return (
    <Card title="采购付款" variant="borderless"
      extra={
        <Space>
          <Input.Search placeholder="搜索单号/入仓单号" allowClear onSearch={v => { setPage(1); setKeyword(v); }} style={{ width: 220 }} />
...
function CreateDrawer({ open, showAmount, onClose, onCreated }: {
  ...
  return (
    <Drawer title="新建采购付款单" width={820} open={open} onClose={onClose}
...
function DetailDrawer({ 单号, showAmount, onClose }: { 单号: string | null; showAmount: boolean; onClose: () => void }) {
  const [detail, setDetail] = useState<PPDetail | null>(null);
  useEffect(() => {
    if (!单号) { setDetail(null); return; }
    (async () => { try { setDetail(await purchasePaymentApi.get(单号)); } catch { message.error("加载付款详情失败"); } })();
  }, [单号]);
  const h = detail?.单头;
  return (
    <Drawer title={`采购付款单 ${单号 ?? ""}`} width={820} open={!!单号} onClose={onClose}>
      {detail && (
        <>
          <Descriptions size="small" column={3} bordered style={{ marginBottom: 16 }}
            items={[
              { key: "no", label: "单号", children: h?.单号 ?? "-" },
              { key: "src", label: "入仓单号", children: h?.入仓单号 ?? "-" },
          ...
          <Table size="small" rowKey="id" pagination={false} dataSource={detail.明细}
            scroll={{ x: "max-content", y: 380 }}
            columns={[
              { title: "供应商编号", dataIndex: "供应商编号" }, { title: "供应商名称", dataIndex: "供应商名称" },
              ...(showAmount ? [

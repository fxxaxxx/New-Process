...
import { semiStocktakeQueryApi, type SemiStkQuerySummaryRow, type SemiStkQueryDetailRow } from "../../api/semi";
import { can } from "../../auth/permissions";
import { usePerms } from "../../auth/PermissionContext";

const MENU = "半成品盘点";
const FIELDS = ["产品装配名称", "产品货号", "产品名称", "配件编号", "客户"];
const diffColor = (v: number) => (v < 0 ? "#cf1322" : v > 0 ? "#389e0d" : undefined);
...
export default function SemiStocktakeQueryPage() {
  ...
  const [summary, setSummary] = useState<SemiStkQuerySummaryRow[]>([]);
  const [detail, setDetail] = useState<SemiStkQueryDetailRow[]>([]);
  ...
  const load = useCallback(async (exact: boolean) => {
    ...
    setLoading(true);
    try {
      if (tab === "summary") setSummary(await semiStocktakeQueryApi.summary({ ...p, materialOnly }));
      else setDetail(await semiStocktakeQueryApi.detail({ ...p, 审核: 审核 || undefined }));
    } catch { message.error("加载半成品盘点查询失败"); }
    finally { setLoading(false); }
  }, [canOpen, tab, range, field, keyword, materialOnly, 审核]);
  useEffect(() => { void load(false); }, [canOpen, tab, range, materialOnly, 审核]); // eslint-disable-line react-hooks/exhaustive-deps

  const shiftMonth = (d: number) => { const b = range[0].add(d, "month"); setRange([b.startOf("month"), b.endOf("month")]); };
  const totals = useMemo(() => summary.reduce((a, r) => ({ sys: a.sys + Number(r.系统数 || 0), cnt: a.cnt + Number(r.盘点数 || 0), diff: a.diff + Number(r.盈亏数 || 0) }), { sys: 0, cnt: 0, diff: 0 }), [summary]);
  const qty = (v: number) => Number(v || 0).toLocaleString();

  const summaryCols: ColumnsType<SemiStkQuerySummaryRow> = [
    { title: "配件编号", dataIndex: "配件编号", width: 130 }, { title: "产品货号", dataIndex: "产品货号", width: 160 },
    { title: "产品名称", dataIndex: "产品名称", width: 180 }, { title: "产品装配名称", dataIndex: "产品装配名称", width: 200 },
    { title: "系统数", dataIndex: "系统数", width: 100, align: "right", render: qty },
    { title: "盘点数", dataIndex: "盘点数", width: 100, align: "right", render: qty },
    { title: "盈亏数", dataIndex: "盈亏数", width: 100, align: "right", render: (v: number) => <span style={{ color: diffColor(v) }}>{qty(v)}</span> },
  ];
  const detailCols: ColumnsType<SemiStkQueryDetailRow> = [
    { title: "日期", dataIndex: "日期", width: 105, render: v => v?.slice(0, 10) }, { title: "单号", dataIndex: "单号", width: 150 },
    ...
    { title: "系统数量", dataIndex: "系统数量", width: 100, align: "right", render: qty },
    { title: "盘点数量", dataIndex: "盘点数量", width: 100, align: "right", render: qty },
    { title: "盈亏数量", dataIndex: "盈亏数量", width: 100, align: "right", render: (v: number) => <span style={{ color: diffColor(v) }}>{qty(v)}</span> },
  ...
  const exportExcel = () => tab === "summary"
    ? csvDownload("半成品盘点查询_汇总.csv", ["配件编号", "产品货号", "产品名称", "产品装配名称", "系统数", "盘点数", "盈亏数"], summary as unknown as Record<string, unknown>[])
    : csvDownload("半成品盘点查询_明细.csv", ["日期", "单号", "配件编号", "产品货号", "产品名称", "产品装配名称", "系统数量", "盘点数量", "盈亏数量", "备注", "审核"], detail as unknown as Record<string, unknown>[]);
  ...
  return <Card variant="borderless"
    title={<Space size={16}><span>半成品盘点查询</span><span style={{ color: "#52c41a", fontSize: 13 }}>查询记录：{rowCount}{tab === "summary" ? `　盈亏合计：${totals.diff.toLocaleString()}` : ""}</span></Space>}
    extra={<Space wrap>
    ...
    <Space wrap style={{ marginBottom: 12 }}>
      ...
      {tab === "detail" && <Space size={4}><span style={{ color: "#8c8c8c" }}>审核情况</span>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: "summary", label: "汇总查询", children:
        <Table<SemiStkQuerySummaryRow> rowKey={(r, i) => `${r.配件编号 ?? ""}|${r.产品货号 ?? ""}|${i}`} size="small" loading={loading} columns={summaryCols} dataSource={summary}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 990, y: "calc(100vh - 360px)" }} /> },
      { key: "detail", label: "明细查询", children:
        <Table<SemiStkQueryDetailRow> rowKey={(r, i) => `${r.单号 ?? ""}|${r.配件编号 ?? ""}|${i}`} size="small" loading={loading} columns={detailCols} dataSource={detail}
          onRow={r => ({ onDoubleClick: () => r.单号 && navigate(`/semi-stocktakes?open=${encodeURIComponent(r.单号)}`), style: { cursor: "pointer" } })}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 1450, y: "calc(100vh - 360px)" }} /> },
    ]} />
  </Card>;
}

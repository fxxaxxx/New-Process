...
import { semiScrapQueryApi, type SemiScrapSummaryRow, type SemiScrapDetailRow } from "../../api/semi";
...
export default function SemiScrapQueryPage() {
  ...
  const [summary, setSummary] = useState<SemiScrapSummaryRow[]>([]);
  const [detail, setDetail] = useState<SemiScrapDetailRow[]>([]);
  ...
  const load = useCallback(async (exact: boolean) => {
    ...
    setLoading(true);
    try {
      if (tab === "summary") setSummary(await semiScrapQueryApi.summary({ ...p, materialOnly, byOrderNo }));
      else setDetail(await semiScrapQueryApi.detail({ ...p, 审核: 审核 || undefined }));
    } catch { message.error("加载半成品报废查询失败"); }
  ...
  const total = useMemo(() => summary.reduce((a, r) => a + Number(r.报废数量 || 0), 0), [summary]);

  const summaryCols: ColumnsType<SemiScrapSummaryRow> = [
    ...
    { title: "报废数量", dataIndex: "报废数量", width: 120, align: "right", render: (v: number) => Number(v || 0).toLocaleString() },
  ];
  const detailCols: ColumnsType<SemiScrapDetailRow> = [
    { title: "日期", dataIndex: "日期", width: 105, render: v => v?.slice(0, 10) }, { title: "单号", dataIndex: "单号", width: 140 },
    { title: "仓库", dataIndex: "仓库", width: 100 }, { title: "报废部门", dataIndex: "报废部门", width: 120 }, { title: "报废人", dataIndex: "报废人", width: 110 },
  ...
  const exportExcel = () => tab === "summary"
    ? csvDownload("半成品报废查询_汇总.csv", ["配件编号", "产品货号", "产品名称", "产品装配名称", "报废数量"], summary as unknown as Record<string, unknown>[])
    : csvDownload("半成品报废查询_明细.csv", ["日期", "单号", "仓库", "报废部门", "报废人", "配件编号", "产品货号", "产品名称", "产品装配名称", "数量", "备注", "审核"], detail as unknown as Record<string, unknown>[]);
  ...
  return <Card variant="borderless"
    title={<Space size={16}><span>半成品报废查询</span><span style={{ color: "#52c41a", fontSize: 13 }}>查询记录：{rowCount}{tab === "summary" ? `　总合计：${total.toLocaleString()}` : ""}</span></Space>}
    extra={<Space wrap>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: "summary", label: "汇总查询", children:
        <Table<SemiScrapSummaryRow> rowKey={(r, i) => `${r.配件编号 ?? ""}|${r.产品货号 ?? ""}|${i}`} size="small" loading={loading} columns={summaryCols} dataSource={summary}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 840, y: "calc(100vh - 360px)" }} /> },
      { key: "detail", label: "明细查询", children:
        <Table<SemiScrapDetailRow> rowKey={(r, i) => `${r.单号 ?? ""}|${r.配件编号 ?? ""}|${i}`} size="small" loading={loading} columns={detailCols} dataSource={detail}
          onRow={r => ({ onDoubleClick: () => r.单号 && navigate(`/semi-scraps?open=${encodeURIComponent(r.单号)}`), style: { cursor: "pointer" } })}

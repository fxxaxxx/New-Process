...
import { semiLabelQueryApi, type SemiLabelSummaryRow, type SemiLabelDetailRow } from "../../api/semi";
...
export default function SemiLabelQueryPage() {
  ...
  const [materialOnly, setMaterialOnly] = useState(true);   // 汇总：物料查询(共用物料)
  const [审核, set审核] = useState("");                       // 明细：审核情况 全部/1/0
  const [summary, setSummary] = useState<SemiLabelSummaryRow[]>([]);
  const [detail, setDetail] = useState<SemiLabelDetailRow[]>([]);
  ...
  const load = useCallback(async (exact: boolean) => {
    ...
    setLoading(true);
    try {
      if (tab === "summary") setSummary(await semiLabelQueryApi.summary({ ...p, materialOnly }));
      else setDetail(await semiLabelQueryApi.detail({ ...p, 审核: 审核 || undefined }));
    } catch { message.error("加载半成品标签查询失败"); }
  ...
  const summaryCols: ColumnsType<SemiLabelSummaryRow> = [
    { title: "配件编号", dataIndex: "配件编号", width: 120 }, { title: "客户", dataIndex: "客户", width: 100 },
    { title: "产品货号", dataIndex: "产品货号", width: 150 }, { title: "产品名称", dataIndex: "产品名称", width: 180 },
    ...
    { title: "每箱数量", dataIndex: "每箱数量", width: 90, align: "right" },
    { title: "预计标签数", dataIndex: "预计标签数", width: 100, align: "right" },
    { title: "实需标签数", dataIndex: "实需标签数", width: 100, align: "right" },
  ];
  const detailCols: ColumnsType<SemiLabelDetailRow> = [
    { title: "日期", dataIndex: "日期", width: 105, render: v => v?.slice(0, 10) }, { title: "单号", dataIndex: "单号", width: 140 },
    { title: "配件编号", dataIndex: "配件编号", width: 120 }, { title: "客户", dataIndex: "客户", width: 100 },
    { title: "产品货号", dataIndex: "产品货号", width: 150 }, { title: "产品名称", dataIndex: "产品名称", width: 170 },
  ...
  const exportExcel = () => tab === "summary"
    ? csvDownload("半成品标签查询_汇总.csv", ["配件编号", "客户", "产品货号", "产品名称", "产品装配名称", "数量", "每箱数量", "预计标签数", "实需标签数"], summary as unknown as Record<string, unknown>[])
    : csvDownload("半成品标签查询_明细.csv", ["日期", "单号", "配件编号", "客户", "产品货号", "产品名称", "产品装配名称", "数量", "每箱数量", "预计标签数", "实需标签数", "备注", "审核"], detail as unknown as Record<string, unknown>[]);
  ...
  return <Card variant="borderless"
    title={<Space size={16}><span>半成品标签查询</span><span style={{ color: "#52c41a", fontSize: 13 }}>查询记录：{rowCount}</span></Space>}
    extra={<Space wrap>
    ...
    <Space wrap style={{ marginBottom: 12 }}>
      ...
      <Input size="small" allowClear value={keyword} onChange={e => setKeyword(e.target.value)} onPressEnter={() => void load(false)} placeholder="输入查询内容" style={{ width: 230 }} />
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: "summary", label: "汇总查询", children:
        <Table<SemiLabelSummaryRow> rowKey={(r, i) => `${r.配件编号 ?? ""}|${r.产品货号 ?? ""}|${i}`} size="small" loading={loading} columns={summaryCols} dataSource={summary}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 1020, y: "calc(100vh - 360px)" }} /> },
      { key: "detail", label: "明细查询", children:
        <Table<SemiLabelDetailRow> rowKey={(r, i) => `${r.单号 ?? ""}|${r.配件编号 ?? ""}|${i}`} size="small" loading={loading} columns={detailCols} dataSource={detail}
          onRow={r => ({ onDoubleClick: () => r.单号 && navigate(`/semi-finished-label-orders?open=${encodeURIComponent(r.单号)}`), style: { cursor: "pointer" } })}

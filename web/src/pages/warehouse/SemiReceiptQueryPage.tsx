...
import { semiReceiptQueryApi, type SemiReceiptSummaryRow, type SemiReceiptDetailRow } from "../../api/semi";
...
export default function SemiReceiptQueryPage() {
  ...
  const [summary, setSummary] = useState<SemiReceiptSummaryRow[]>([]);
  const [detail, setDetail] = useState<SemiReceiptDetailRow[]>([]);
  ...
  const load = useCallback(async (exact: boolean) => {
    ...
    setLoading(true);
    try {
      if (tab === "summary") setSummary(await semiReceiptQueryApi.summary({ ...p, materialOnly, bySupplier }));
      else setDetail(await semiReceiptQueryApi.detail({ ...p, 审核: 审核 || undefined }));
    } catch { message.error("加载半成品入仓查询失败"); }
  ...
  const total = useMemo(() => summary.reduce((a, r) => a + Number(r.入仓数量 || 0), 0), [summary]);

  const summaryCols: ColumnsType<SemiReceiptSummaryRow> = [
    ...
    { title: "入仓数量", dataIndex: "入仓数量", width: 120, align: "right", render: (v: number) => Number(v || 0).toLocaleString() },
  ];
  const detailCols: ColumnsType<SemiReceiptDetailRow> = [
    ...
    { title: "生产单号", dataIndex: "生产单号", width: 110 }, { title: "配件编号", dataIndex: "配件编号", width: 110 },
    { title: "产品货号", dataIndex: "产品货号", width: 150 }, { title: "产品名称", dataIndex: "产品名称", width: 160 },
  ...
  const exportExcel = () => tab === "summary"
    ? csvDownload("半成品入仓查询_汇总.csv", ["配件编号", "供应商名称", "产品货号", "产品名称", "产品装配名称", "入仓数量"], summary as unknown as Record<string, unknown>[])
    : csvDownload("半成品入仓查询_明细.csv", ["日期", "单号", "入库单号", "订单单号", "供应商编号", "供应商名称", "生产单号", "配件编号", "产品货号", "产品名称", "产品装配名称", "数量", "备注", "审核"], detail as unknown as Record<string, unknown>[]);
  ...
  return <Card variant="borderless"
    title={<Space size={16}><span>半成品入仓查询</span><span style={{ color: "#52c41a", fontSize: 13 }}>查询记录：{rowCount}{tab === "summary" ? `　总合计：${total.toLocaleString()}` : ""}</span></Space>}
    extra={<Space wrap>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: "summary", label: "汇总查询", children:
        <Table<SemiReceiptSummaryRow> rowKey={(r, i) => `${r.配件编号 ?? ""}|${r.产品货号 ?? ""}|${r.供应商编号 ?? ""}|${i}`} size="small" loading={loading} columns={summaryCols} dataSource={summary}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 900, y: "calc(100vh - 360px)" }}
          summary={() => <Table.Summary fixed><Table.Summary.Row><Table.Summary.Cell index={0} colSpan={summaryCols.length - 1}><b>总合计</b></Table.Summary.Cell><Table.Summary.Cell index={1} align="right"><b>{total.toLocaleString()}</b></Table.Summary.Cell></Table.Summary.Row></Table.Summary>} /> },
      { key: "detail", label: "明细查询", children:
        <Table<SemiReceiptDetailRow> rowKey={(r, i) => `${r.单号 ?? ""}|${r.配件编号 ?? ""}|${i}`} size="small" loading={loading} columns={detailCols} dataSource={detail}
          onRow={r => ({ onDoubleClick: () => r.单号 && navigate(`/semi-receipts?open=${encodeURIComponent(r.单号)}`), style: { cursor: "pointer" } })}
          pagination={{ pageSize: 50, showTotal: t => `共 ${t} 条` }} scroll={{ x: 1650, y: "calc(100vh - 360px)" }} /> },
    ]} />
  </Card>;
}

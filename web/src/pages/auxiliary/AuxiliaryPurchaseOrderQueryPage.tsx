...
import {
  auxiliaryPurchaseOrderQueryApi,
  type AuxiliaryPurchaseOrderQueryDetailRow,
  type AuxiliaryPurchaseOrderQuerySummaryRow,
...
import {
  ...
  type AuxiliaryPurchaseOrderAuditStatus,
  type AuxiliaryPurchaseOrderDateMode,
...
type SearchField = "辅料编号" | "辅料名称" | "规格" | "单号" | "供应商名称";
...
export default function AuxiliaryPurchaseOrderQueryPage() {
  const perms = usePerms();
  const canOpen = can(perms, MENU, "打开");
  const [tab, setTab] = useState<TabKey>("summary");
  const [dateMode, setDateMode] = useState<AuxiliaryPurchaseOrderDateMode>("订货日期");
  ...
  const [auditStatus, setAuditStatus] = useState<AuxiliaryPurchaseOrderAuditStatus>("全部");
  const [summaryRows, setSummaryRows] = useState<AuxiliaryPurchaseOrderQuerySummaryRow[]>([]);
  const [detailRows, setDetailRows] = useState<AuxiliaryPurchaseOrderQueryDetailRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [selectedKey, setSelectedKey] = useState<string>();
  const [viewOrderNo, setViewOrderNo] = useState<string>();

  const query = useMemo(() => buildAuxiliaryPurchaseOrderQuery({
    dateMode,
  ...
  const load = useCallback(async (exactMode = false) => {
    ...
    try {
      if (tab === "summary") {
        const result = await auxiliaryPurchaseOrderQueryApi.summary(query);
        const mapped = result.map(normalizeAuxiliaryPurchaseOrderSummaryRow);
        setSummaryRows(mapped);
        setSelectedKey(mapped[0] ? summaryKey(mapped[0], 0) : undefined);
      } else {
        const result = await auxiliaryPurchaseOrderQueryApi.detail(query);
        const mapped = result.map(normalizeAuxiliaryPurchaseOrderDetailRow);
        setDetailRows(mapped);
        setSelectedKey(mapped[0] ? detailKey(mapped[0], 0) : undefined);
      }
    } catch {
      message.error("加载辅料采购订单查询失败");
    } finally {
      setLoading(false);
    }
  }, [canOpen, query, tab]);
  ...
  const visibleSummary = useMemo(() => {
    ...
    return summaryRows.filter(row => {
      const value = String(row[searchField as keyof AuxiliaryPurchaseOrderQuerySummaryRow] ?? "");
  ...
  const visibleDetail = useMemo(() => {
    ...
    return detailRows.filter(row => {
      const value = String(row[searchField as keyof AuxiliaryPurchaseOrderQueryDetailRow] ?? "");
      return exact ? value === kw : value.includes(kw);
    });
  }, [detailRows, exact, keyword, searchField]);

  const supplierColumns: ColumnsType<AuxiliaryPurchaseOrderQuerySummaryRow> = groupBySupplier ? [
    { title: "供应商编号", dataIndex: "供应商编号", width: 128 },
    { title: "供应商名称", dataIndex: "供应商名称", width: 210 },
  ] : [];

  const summaryColumns: ColumnsType<AuxiliaryPurchaseOrderQuerySummaryRow> = [
    {
      title: "",
      key: "selector",
      width: 34,
    ...
    { title: "订货数量", dataIndex: "订货数量", width: 122, align: "right", render: fmtNumber },
  ];

  const detailColumns: ColumnsType<AuxiliaryPurchaseOrderQueryDetailRow> = [
    {
      title: "",
      key: "selector",
      width: 34,
    ...
    { title: "交货日期", dataIndex: "交货日期", width: 100 },
  ...
  if (!canOpen) {
    return (
      <Card variant="borderless">
        <div style={{ padding: 24, color: "#999" }}>无权限访问该页面（缺少“辅料采购订单查询·打开”权限）。</div>
      </Card>
    );
  }
  ...
  return (
    ...
    >
      <div style={auxiliaryReportFilterPanelStyle}>
          <Space wrap size={8} style={auxiliaryReportFilterRowStyle}>
            ...
            {tab === "detail" ? (
              <Space size={8}>
                <span>审核情况：</span>
                <Select<AuxiliaryPurchaseOrderAuditStatus>
                  value={auditStatus}
                  onChange={setAuditStatus}
                  style={{ width: 112 }}
                  options={["全部", "已审核", "未审核"].map(value => ({ value: value as AuxiliaryPurchaseOrderAuditStatus, label: value }))}
                />
              </Space>
            ) : null}
          </Space>
          <Space wrap size={8} style={auxiliaryReportFilterRowStyle}>
            <span>日期</span>
            <Select<AuxiliaryPurchaseOrderDateMode>
              value={dateMode}
              onChange={setDateMode}
              style={{ width: 112 }}
              options={["订货日期", "交货日期", "不选择日期"].map(value => ({ value: value as AuxiliaryPurchaseOrderDateMode, label: value }))}
            />
            <DatePicker
              allowClear={false}
              value={range[0]}
            ...
            <Select<SearchField>
              ...
              style={{ width: 116 }}
              options={["辅料编号", "辅料名称", "规格", "单号", "供应商名称"].map(value => ({ value: value as SearchField, label: value }))}
            />
            <span>查询</span>
            <Input
              allowClear
        ...
        items={[
          {
            ...
            label: "汇总查询",
            children: (
            <Table<AuxiliaryPurchaseOrderQuerySummaryRow>
              rowKey={summaryKey}
              size="small"
              loading={loading}
          ...
          },
          {
            ...
            label: "明细查询",
            children: (
            <Table<AuxiliaryPurchaseOrderQueryDetailRow>
              rowKey={detailKey}
              size="small"
              loading={loading}
              ...
              onRow={(row, index) => ({
                ...
                onDoubleClick: () => row.单号 && setViewOrderNo(row.单号),
                style: {
      ...
      />
      <PurchaseOrderDrawer
        open={viewOrderNo !== undefined}
        单号={viewOrderNo}
        onClose={() => setViewOrderNo(undefined)}
      />
    </AuxiliaryReportLayout>
  );
}

function summaryKey(row: AuxiliaryPurchaseOrderQuerySummaryRow, index?: number) {
  return [
    row.供应商编号 ?? "",
    row.辅料编号 ?? "",
    row.规格 ?? "",
...
function detailKey(row: AuxiliaryPurchaseOrderQueryDetailRow, index?: number) {
  return [
    row.单号 ?? "",
    row.辅料编号 ?? "",
    row.规格 ?? "",

...
import { salesShipmentApi, type SSDetail, type SSHeader, type SSLine } from "../../api/sales";
...
export default function SalesShipmentPage() {
  ...
  const load = useCallback(async () => {
    try { const r = await salesShipmentApi.list(page, 10, keyword); setRows(r.items); setTotal(r.total); }
    catch { message.error("加载销售出货单失败"); }
  }, [page, keyword]);
  useEffect(() => { load(); }, [load]);
  ...
  const columns = [
    ...
    { title: "出货数量", dataIndex: "数量", key: "数量" },
      ...
      render: (v?: string) => v === "1" ? <Tag color="green" style={{ borderRadius: 6 }}>已审核</Tag> : <Tag style={{ borderRadius: 6 }}>未审核</Tag> },
    {
      ...
      render: (_: unknown, row: SSHeader) => (
        <Space>
          {row.审核 !== "1" && can(perms, MENU, "审核") && <a onClick={() => act(() => salesShipmentApi.approve(row.单号!), "已审核")}>审核</a>}
          {row.审核 === "1" && can(perms, MENU, "反审核") && <a onClick={() => act(() => salesShipmentApi.unapprove(row.单号!), "已反审核")}>反审核</a>}
          {row.审核 !== "1" && can(perms, MENU, "删除") && (
            <Popconfirm title="确认删除该出货单?" onConfirm={() => act(() => salesShipmentApi.remove(row.单号!), "已删除")}><a>删除</a></Popconfirm>
          )}
        </Space>
      ),
    },
  ...
  return (
    <Card title="销售出货" variant="borderless"
      extra={
        <Space>
...
function CreateDrawer({ open, showPrice, onClose, onCreated }: {
  ...
  const [form] = Form.useForm<{ 仓库: string; 客户编号?: string; 客户名称?: string; 付款方式?: string; 备注?: string }>();
  const [lines, setLines] = useState<SSLine[]>([]);
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (!open) return; form.resetFields(); setLines([]); }, [open, form]);

  const setLine = (i: number, patch: Partial<SSLine>) =>
    setLines(prev => prev.map((l, j) => (j === i ? { ...l, ...patch } : l)));

  const submit = async () => {
    let v: { 仓库: string; 客户编号?: string; 客户名称?: string; 付款方式?: string; 备注?: string };
    try { v = await form.validateFields(); } catch { return; }
    const ok = lines.filter(l => !!l.物料编号 && Number(l.数量) > 0);
    if (ok.length === 0) { message.error("请至少录入一行有物料和数量的明细"); return; }
    setSaving(true);
    try {
      await salesShipmentApi.create({ ...v, 明细: ok });
      message.success("销售出货单已创建"); onClose(); onCreated();
    } catch (e) {
      message.error((e as { response?: { data?: { 消息?: string } } }).response?.data?.消息 ?? "创建出货单失败");
    } finally { setSaving(false); }
  };

  const columns = [
    { title: "物料编号", dataIndex: "物料编号", width: 130, render: (_: unknown, r: SSLine, i: number) =>
      <Input style={{ width: 116 }} value={r.物料编号 ?? ""} onChange={e => setLine(i, { 物料编号: e.target.value })} /> },
    { title: "物料名称", dataIndex: "物料名称", width: 130, render: (_: unknown, r: SSLine, i: number) =>
      <Input style={{ width: 116 }} value={r.物料名称 ?? ""} onChange={e => setLine(i, { 物料名称: e.target.value })} /> },
    { title: "规格", dataIndex: "规格", width: 100, render: (_: unknown, r: SSLine, i: number) =>
      <Input style={{ width: 88 }} value={r.规格 ?? ""} onChange={e => setLine(i, { 规格: e.target.value })} /> },
    { title: "颜色", dataIndex: "颜色", width: 90, render: (_: unknown, r: SSLine, i: number) =>
      <Input style={{ width: 80 }} value={r.颜色 ?? ""} onChange={e => setLine(i, { 颜色: e.target.value })} /> },
    { title: "单位", dataIndex: "单位", width: 80, render: (_: unknown, r: SSLine, i: number) =>
      <Input style={{ width: 68 }} value={r.单位 ?? ""} onChange={e => setLine(i, { 单位: e.target.value })} /> },
    { title: "数量", dataIndex: "数量", width: 100, render: (_: unknown, r: SSLine, i: number) =>
      <InputNumber min={0} precision={0} style={{ width: 88 }} value={r.数量 ?? 0} onChange={n => setLine(i, { 数量: Number(n ?? 0) })} /> },
    ...(showPrice ? [{ title: "单价", dataIndex: "单价", width: 110, render: (_: unknown, r: SSLine, i: number) =>
      <InputNumber min={0} style={{ width: 96 }} value={r.单价 ?? 0} onChange={n => setLine(i, { 单价: Number(n ?? 0) })} /> }] : []),
    ...(showPrice ? [{ title: "金额", key: "金额", width: 100, render: (_: unknown, r: SSLine) => Number(r.数量 ?? 0) * Number(r.单价 ?? 0) }] : []),
    { title: "", key: "_op", width: 50, render: (_: unknown, __: SSLine, i: number) =>
      <a onClick={() => setLines(prev => prev.filter((_, j) => j !== i))}>删除</a> },
  ];

  return (
    <Drawer title="新建销售出货单" width={1040} open={open} onClose={onClose}
      ...
      <Form form={form} layout="vertical">
        ...
        <Row gutter={16}>
          <Col span={8}><Form.Item name="付款方式" label="付款方式"><Input /></Form.Item></Col>
      ...
      <Space style={{ marginTop: 12 }} size={24}>
        ...
        <Statistic title="出货数量合计" value={sumQty(lines)} />
...
function DetailDrawer({ 单号, showPrice, onClose }: { 单号: string | null; showPrice: boolean; onClose: () => void }) {
  const [detail, setDetail] = useState<SSDetail | null>(null);
  useEffect(() => {
    if (!单号) { setDetail(null); return; }
    (async () => { try { setDetail(await salesShipmentApi.get(单号)); } catch { message.error("加载出货详情失败"); } })();
  }, [单号]);
  const h = detail?.单头;
  return (
    <Drawer title={`销售出货单 ${单号 ?? ""}`} width={900} open={!!单号} onClose={onClose}>
      {detail && (
        <>
          <Descriptions size="small" column={3} bordered style={{ marginBottom: 16 }}
            items={[
              ...
              { key: "qty", label: "出货数量", children: String(h?.数量 ?? "-") },

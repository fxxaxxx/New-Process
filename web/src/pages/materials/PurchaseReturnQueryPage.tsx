...
import {
  purchaseReturnQueryApi,
  type PurchaseReturnQueryDetailRow,
  type PurchaseReturnSummaryRow,
} from "../../api/purchaseReturnQuery";
...
export default function PurchaseReturnQueryPage() {
  ...
  const [detail, setDetail] = useState<PurchaseReturnQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<PurchaseReturnSummaryRow[]>([]);
  ...
  const summaryColumns = [
    ...
    { title: "颜色", dataIndex: "颜色" },
    { title: "单位", dataIndex: "单位" },
    { title: "退仓数量", dataIndex: "退仓数量", align: "right" as const,
      render: (v: number) => <span style={{ fontWeight: 600 }}>{v}</span> },
  ];

  const detailExportCols: ExportCol[] = [
    { title: "日期", key: "日期", fmt: v => String(v ?? "").slice(0, 10) },
    { title: "单号", key: "单号" }, { title: "供应商编号", key: "供应商编号" },
  ...
  const summaryExportCols: ExportCol[] = [
    ...
    { title: "颜色", key: "颜色" }, { title: "单位", key: "单位" }, { title: "退仓数量", key: "退仓数量" },
  ];
  ...
  return (
    <Card title="采购退仓单查询" variant="borderless">
      <div>

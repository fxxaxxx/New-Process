...
import {
  materialIssueQueryApi,
  type MaterialIssueQueryDetailRow,
  type MaterialIssueSummaryRow,
} from "../../api/materialIssueQuery";
...
export default function MaterialIssueQueryPage() {
  ...
  const [detail, setDetail] = useState<MaterialIssueQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<MaterialIssueSummaryRow[]>([]);
  ...
  const load = useCallback(async (silent = false) => {
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await materialIssueQueryApi.detail(query));
      else setSummary(await materialIssueQueryApi.summary(query));
    } catch { if (!silent) message.error("加载领料单查询失败"); }
  ...
  const detailColumns = [
    ...
    { title: "领料部门", dataIndex: "领料部门" },
    { title: "领料人", dataIndex: "领料人" },
  ...
  const summaryColumns = [
    ...
    { title: "颜色", dataIndex: "颜色" },
    { title: "单位", dataIndex: "单位" },
    { title: "领用数量", dataIndex: "领用数量", align: "right" as const,
  ...
  const detailExportCols: ExportCol[] = [
    ...
    { title: "单号", key: "单号" }, { title: "生产单号", key: "生产单号" },
    { title: "款号", key: "款号" }, { title: "领料部门", key: "领料部门" },
    { title: "领料人", key: "领料人" }, { title: "物料编号", key: "物料编号" },
  ...
  const summaryExportCols: ExportCol[] = [
    ...
    { title: "颜色", key: "颜色" }, { title: "单位", key: "单位" }, { title: "领用数量", key: "领用数量" },
  ];
  ...
  return (
    <Card title="领料单查询" variant="borderless">
      <div>
        <Space style={{ marginBottom: 12 }} wrap>
          ...
          <Input.Search placeholder="单号/生产单号/款号/领料人/物料" allowClear onSearch={setKeyword} style={{ width: 240 }} />
      ...
      <MaterialDocDetailDrawer cfg={ISSUE_CFG} 单号={viewing} onClose={() => setViewing(null)} />
    </Card>
  );
}

...
import {
  ...
  type MaterialLabelSummaryRow,
} from "../../api/materialLabel";
...
export default function MaterialLabelQueryPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await materialLabelApi.detail(query));
      else setSummary(await materialLabelApi.summary(query));
    } catch { message.error("加载来料标签查询失败"); }
  ...
  return (
    <Card title="来料标签查询" variant="borderless">
      <div>

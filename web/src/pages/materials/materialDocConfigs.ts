...
// operator=只读操作员(当前用户)；employee=输入框+🔍选人(人事档案)；recipient=接受人下拉(职称=仓管/PMC)
export type DocFieldType = "text" | "date-today" | "docno" | "operator" | "employee" | "select" | "recipient";
export interface DocFieldCfg { name: string; label: string; required?: boolean; type?: DocFieldType; noCopy?: boolean; defaultValue?: string; options?: string[] }
export interface MaterialDocCfg {
  ...
  outbound?: boolean;      // true=支持分次出库(领料单):未审核单可「出库」,明细显示 已出/未领
}
...
export const MATERIAL_DOC_CONFIGS: Record<string, MaterialDocCfg> = {
  "purchase-receipts": {
    ...
    // 单号=供应商送货单号：手填必填，唯一（同一张送货单不能重复入仓）；复制单不带出(noCopy)
    headerFields: [
      { name: "单号", label: "送货单号", required: true, noCopy: true },
  ...
  "material-issues": {
    ...
    listExtra: [{ name: "领料部门", label: "领料部门" }, { name: "领料人", label: "领料人" }, { name: "仓库", label: "仓库" }, { name: "接受人", label: "接受人" }],
  },
  "material-returns": {

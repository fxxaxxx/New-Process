...
import {
  materialReturnQueryApi,
  type MaterialReturnQueryDetailRow,
  type MaterialReturnSummaryRow,
} from "../../api/materialReturnQuery";
...
export default function MaterialReturnQueryPage() {
  ...
  const [detail, setDetail] = useState<MaterialReturnQueryDetailRow[]>([]);
  const [summary, setSummary] = useState<MaterialReturnSummaryRow[]>([]);
  ...
  const load = useCallback(async (silent = false) => {
    setLoading(true);
    try {
      if (tab === "detail") setDetail(await materialReturnQueryApi.detail(query));
      else setSummary(await materialReturnQueryApi.summary(query));
    } catch { if (!silent) message.error("加载退料单查询失败"); }
  ...
  const detailColumns = [
    ...
    { title: "退料部门", dataIndex: "退料部门" },
    { title: "退料人", dataIndex: "退料人" },
  ...
  const summaryColumns = [
    ...
    { title: "颜色", dataIndex: "颜色" },
    { title: "单位", dataIndex: "单位" },
    { title: "退料数量", dataIndex: "退料数量", align: "right" as const,
  ...
  const detailExportCols: ExportCol[] = [
    ...
    { title: "单号", key: "单号" }, { title: "退料部门", key: "退料部门" },
    { title: "退料人", key: "退料人" }, { title: "物料编号", key: "物料编号" },
  ...
  const summaryExportCols: ExportCol[] = [
    ...
    { title: "颜色", key: "颜色" }, { title: "单位", key: "单位" }, { title: "退料数量", key: "退料数量" },
  ];
  ...
  return (
    <Card title="退料单查询" variant="borderless">
      <div>
        <Space style={{ marginBottom: 12 }} wrap>
          ...
          <Input.Search placeholder="单号/生产单号/款号/退料人/物料" allowClear onSearch={setKeyword} style={{ width: 240 }} />
      ...
      <MaterialDocDetailDrawer cfg={RETURN_CFG} 单号={viewing} onClose={() => setViewing(null)} />
    </Card>
  );
}

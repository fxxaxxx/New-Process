...
export interface MaterialLabelMaterialQuery {
  page?: number;
  size?: number;
  field?: string;
  keyword?: string;
...
const base = "/material-label-orders";
const enc = encodeURIComponent;

export const materialLabelOrdersApi = {
  list: (page = 1, size = 20, keyword = "") =>
    api.get<Paged<MaterialLabelOrderListRow>>(base, { params: { page, size, keyword } })
      // 后端按 camelCase 序列化为 id，这里归一化为 ID（与全项目调用方一致）
      .then(r => ({ ...r.data, items: r.data.items.map(x => ({ ...x, ID: (x as unknown as { id?: number }).id ?? x.ID })) })),
  get: (电脑单号: string) => api.get<MaterialLabelOrder>(`${base}/${enc(电脑单号)}`).then(r => r.data),
  create: (body: MaterialLabelOrderSave) => api.post<{ 电脑单号: string }>(base, body).then(r => r.data),
  update: (电脑单号: string, body: MaterialLabelOrderSave) =>
    api.put<MaterialLabelOrder>(`${base}/${enc(电脑单号)}`, body).then(r => r.data),
  ...
  adjacent: (电脑单号: string, direction: AdjacentDirection) =>
    api.get<MaterialLabelOrder | undefined>(`${base}/${enc(电脑单号)}/adjacent`, { params: { direction } })
      .then(r => r.status === 204 ? undefined : r.data),
  materials: (params: MaterialLabelMaterialQuery = {}) =>
    api.get<Paged<MaterialLabelMaterialRow>>(`${base}/materials`, { params }).then(r => r.data),
};

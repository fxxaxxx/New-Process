export interface FinishedReceiptEditableLine {
  key: number;
  订单单号?: string;
  配件编号: string;
  客户?: string | null;
...
type Product = Omit<FinishedReceiptEditableLine, "key" | "数量"> & { 数量?: number | null };

export function mergeFinishedReceiptProducts(current: FinishedReceiptEditableLine[], selected: Product[]) {
...
export function summarizeFinishedReceiptLines(lines: FinishedReceiptEditableLine[]) {
...
export function validateFinishedReceipt(value: { 仓库?: string; 明细: FinishedReceiptEditableLine[] }) {

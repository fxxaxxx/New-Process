...
export function mergeSemiScrapLines(existing: SSDraftLine[], picked: PickedProduct[]): SSDraftLine[] {
...
export function validateSemiScrap(input: { 明细: SSDraftLine[] }): string | null {
  const valid = input.明细.filter(l => l.配件编号.trim());
  if (valid.length === 0) return "请至少录入一行报废产品。";
  for (const l of valid) if (Number(l.数量) <= 0) return "报废数量必须大于 0。";

...
// 采购付款 REST（供应商级挂账，冲应付，不碰库存）。审核仅翻单头(明细无审核列)，无 SyncLineApprovalAsync。
// 成本保密按「金额」权限剥离 货款金额/付款金额/尚欠金额（付款单无单价列）。
[ApiController]
[Authorize]
[Route("api/purchase-payments")]
public sealed class PurchasePaymentController(
    PurchasePaymentService svc, IPostingEngine posting, IPermissionService perms,
    IAuditLogger audit, ISqlConnectionFactory factory) : ControllerBase
{
    private const string Menu = "采购付款";
    private const string Table = "采购付款单";
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] PurchasePaymentCreateDto dto)
    {
        if (!await AllowAsync(PermissionAction.保存)) return Forbid();
        string 单号;
        try { 单号 = await svc.CreateAsync(dto, CurrentUser); }
        catch (ArgumentException ex) { return BadRequest(new { 消息 = ex.Message }); }
        catch (SqlException ex) when (ex.Number == 547) { return BadRequest(new { 消息 = "供应商不存在。" }); }

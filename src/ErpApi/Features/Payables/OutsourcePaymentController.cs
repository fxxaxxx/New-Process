...
// 发外加工付款 REST（加工厂级挂账，冲应付，不碰库存）。审核仅翻单头(明细无审核列)，无 SyncLineApprovalAsync。
// 成本保密按「金额」权限剥离 货款金额/付款金额/尚欠金额（付款单无单价列）。
[ApiController]
[Authorize]
[Route("api/outsource-payments")]
public sealed class OutsourcePaymentController(
    OutsourcePaymentService svc, IPostingEngine posting, IPermissionService perms,
    IAuditLogger audit, ISqlConnectionFactory factory) : ControllerBase
{
    private const string Menu = "发外付款";
    private const string Table = "发外加工付款单";
    ...
    public async Task<IActionResult> Get(string 单号)
    {
        ...
        if (!await AllowAsync(PermissionAction.金额))
            foreach (var l in d.明细) { l.货款金额 = null; l.付款金额 = null; l.尚欠金额 = null; }
        return Ok(d);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] OutsourcePaymentCreateDto dto)
    {
        if (!await AllowAsync(PermissionAction.保存)) return Forbid();
        string 单号;
        try { 单号 = await svc.CreateAsync(dto, CurrentUser); }
        catch (ArgumentException ex) { return BadRequest(new { 消息 = ex.Message }); }
        catch (SqlException ex) when (ex.Number == 547) { return BadRequest(new { 消息 = "加工厂不存在。" }); }

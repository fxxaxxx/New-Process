...
// 销售退货 REST（纯应收逆向，不进库存账本）。审核仅翻单头(明细无审核列)，无 SyncLineApprovalAsync。缺单价权限剥离金额列。
[ApiController]
[Authorize]
[Route("api/sales-returns")]
public sealed class SalesReturnController(
    SalesReturnService svc, IPostingEngine posting, IPermissionService perms,
    IAuditLogger audit, ISqlConnectionFactory factory) : ControllerBase
{
    private const string Menu = "销售退货";
    private const string Table = "销售退货单";

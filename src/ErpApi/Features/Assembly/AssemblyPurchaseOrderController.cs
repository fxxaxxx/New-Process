...
[ApiController]
[Authorize]
[Route("api/assembly-purchase-orders")]
public sealed class AssemblyPurchaseOrderController(
    AssemblyPurchaseOrderService svc, IPostingEngine posting, IPermissionService perms) : ControllerBase
{
    private const string Menu = "装配加工采购单";
    private const string Table = "装配加工采购单";
    private string CurrentUser => User.FindFirstValue(ClaimTypes.NameIdentifier) ?? User.FindFirstValue("sub") ?? "";
    private Task<bool> AllowAsync(PermissionAction a) => perms.HasAsync(CurrentUser, Menu, a);

    [HttpGet]
    public async Task<IActionResult> List(int page = 1, int size = 50, string? keyword = null)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] AssemblyPurchaseOrderSaveDto dto)
    {
    ...
    [HttpPut("{单号}")]
    public async Task<IActionResult> Update(string 单号, [FromBody] AssemblyPurchaseOrderSaveDto dto)
    {
        if (!await AllowAsync(PermissionAction.保存)) return Forbid();

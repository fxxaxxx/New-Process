...
[ApiController]
[Authorize]
[Route("api/material-label-orders")]
public sealed class MaterialLabelOrderController(
    IMaterialLabelOrderService svc,
    IPermissionService perms) : ControllerBase
{
    private const string Menu = "来料标签单";
    // 来料标签查询(报表)走独立权限菜单,与单据本身权限分开。
    private const string QueryMenu = "来料标签查询";
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] MaterialLabelOrderSaveDto dto)
    {
    ...
    [HttpPut("{documentNo}")]
    public async Task<IActionResult> Update(string documentNo, [FromBody] MaterialLabelOrderSaveDto dto)
    {
        if (!await AllowAsync(PermissionAction.保存)) return Forbid();
        if (string.IsNullOrWhiteSpace(documentNo)) return BadRequest(new { 消息 = "电脑单号必填。" });
        try
    ...
    [HttpGet("materials")]
    public async Task<IActionResult> Materials([FromQuery] MaterialLabelMaterialQuery query)
    {
        if (!await AllowAsync(PermissionAction.打开)) return Forbid();
    ...
    // 来料标签查询·明细：每行一条标签明细(双击 电脑单号 看整单)。无价格列。
    ...
    // 来料标签查询·汇总：按 物料编号+规格+颜色 合并 数量与标签数。

...
public interface IMaterialLabelOrderService
{
    Task<MaterialLabelOrderDto> CreateAsync(MaterialLabelOrderSaveDto dto, string user);
    Task<MaterialLabelOrderDto> UpdateAsync(string documentNo, MaterialLabelOrderSaveDto dto, string user);
    Task<MaterialLabelOrderDto?> GetAsync(string documentNo);
    Task<PagedResult<MaterialLabelOrderListRow>> ListAsync(int page, int size, string? keyword);
    Task<bool> DeleteAsync(string documentNo, string user);
    Task<bool> SetAuditAsync(string documentNo, bool audited, string user);
    Task<MaterialLabelOrderDto?> GetAdjacentAsync(string documentNo, AdjacentDirection direction);
    Task<PagedResult<MaterialLabelMaterialRow>> MaterialsAsync(MaterialLabelMaterialQuery query, bool canSeePrice);
    Task<IReadOnlyList<MaterialLabelQueryDetailRow>> LabelQueryDetailAsync(
        DateTime? 起, DateTime? 止, string? keyword, string? 物料类别, string? 审核情况);
    Task<IReadOnlyList<MaterialLabelQuerySummaryRow>> LabelQuerySummaryAsync(
        DateTime? 起, DateTime? 止, string? keyword, string? 物料类别, string? 审核情况);
}

public sealed class MaterialLabelOrderService(
    ISqlConnectionFactory factory,
    IDocumentNumberGenerator docNo,
    IAuditLogger audit) : IMaterialLabelOrderService
{
    public const string DocType = "来料标签单";
    public const string Prefix = "LLB";
    private const decimal Decimal18_4Max = 99_999_999_999_999.9999m;

    public async Task<MaterialLabelOrderDto> CreateAsync(MaterialLabelOrderSaveDto dto, string user)
    {
        ValidateUser(user);
...
INSERT INTO [来料标签单]([电脑单号],[日期],[备注一],[备注二],[操作员],[审核])
...
SELECT CAST(SCOPE_IDENTITY() AS bigint);",
    ...
    public async Task<MaterialLabelOrderDto> UpdateAsync(string documentNo, MaterialLabelOrderSaveDto dto, string user)
    {
        ValidateUser(user);
        using var c = factory.Create();
        await c.OpenAsync();
        ...
        try
        {
            ...
            if (header is null) throw new KeyNotFoundException($"来料标签单 [{documentNo}] 不存在。");
            if (header.审核 == "1") throw new InvalidOperationException("已审核的来料标签单不能保存，请先反审核。");
            var lines = ValidateAndNormalize(dto);
...
WHERE [ID]=@id;", new { 日期 = dto.日期.Date, dto.备注一, dto.备注二, 操作员 = user, id = header.ID }, tx);
            await c.ExecuteAsync("DELETE FROM [来料标签明细] WHERE [标签单ID]=@id", new { id = header.ID }, tx);
    ...
    public async Task<MaterialLabelOrderDto?> GetAsync(string documentNo)
    {
        using var c = factory.Create();
        await c.OpenAsync();
        using var tx = c.BeginTransaction(IsolationLevel.RepeatableRead);
        MaterialLabelOrderDto? header;
        using (var multi = await c.QueryMultipleAsync(@"
SELECT [ID],[电脑单号],[日期],[备注一],[备注二],[操作员],[审核],[审核人],[审核时间]
FROM [来料标签单] WHERE [电脑单号]=@documentNo;
SELECT d.[ID],d.[物料编号],d.[物料名称],d.[规格],d.[颜色],d.[单位],
       d.[数量],d.[标签数],d.[备注]
FROM [来料标签明细] d
INNER JOIN [来料标签单] h ON h.[ID]=d.[标签单ID]
WHERE h.[电脑单号]=@documentNo ORDER BY d.[行号];", new { documentNo }, tx))
        {
            header = await multi.ReadFirstOrDefaultAsync<MaterialLabelOrderDto>();
            if (header is not null)
                header.明细 = (await multi.ReadAsync<MaterialLabelOrderLineDto>()).AsList();
        }
        tx.Commit();
        return header;
    }

    public async Task<PagedResult<MaterialLabelOrderListRow>> ListAsync(int page, int size, string? keyword)
    {
...
OFFSET (@page-1)*@size ROWS FETCH NEXT @size ROWS ONLY;", new { page, size, match });
        ...
        var items = (await multi.ReadAsync<MaterialLabelOrderListRow>()).AsList();
        return new PagedResult<MaterialLabelOrderListRow>(items, total);
    }
    ...
    public async Task<bool> DeleteAsync(string documentNo, string user)
    {
        ...
        try
        {
            ...
            if (header.审核 == "1") throw new InvalidOperationException("已审核的来料标签单不能删除，请先反审核。");
            await c.ExecuteAsync("DELETE FROM [来料标签单] WHERE [ID]=@id", new { id = header.ID }, tx);
...
WHERE [ID]=@id;", new { 审核 = audited ? "1" : "0", user, id = header.ID }, tx);
    ...
    public async Task<MaterialLabelOrderDto?> GetAdjacentAsync(string documentNo, AdjacentDirection direction)
    {
        using var c = factory.Create();
        await c.OpenAsync();
        var current = await c.QuerySingleOrDefaultAsync<HeaderOrderRow>(
            "SELECT [ID],[日期] FROM [来料标签单] WHERE [电脑单号]=@documentNo", new { documentNo });
        if (current is null) return null;
        var next = direction == AdjacentDirection.Next;
        var adjacentNo = await c.ExecuteScalarAsync<string?>(next ? @"
SELECT TOP (1) [电脑单号] FROM [来料标签单]
...
ORDER BY [日期] DESC,[ID] DESC;", current);
    ...
    public async Task<PagedResult<MaterialLabelMaterialRow>> MaterialsAsync(
        MaterialLabelMaterialQuery query, bool canSeePrice)
    {
...
OFFSET (@page-1)*@size ROWS FETCH NEXT @size ROWS ONLY;";
        ...
        var total = await multi.ReadFirstAsync<int>();
        var items = (await multi.ReadAsync<MaterialLabelMaterialRow>()).AsList();
        if (!canSeePrice)
            foreach (var item in items) item.单价 = null;
        return new PagedResult<MaterialLabelMaterialRow>(items, total);
    }
    ...
    // 来料标签查询·明细：每行一条标签明细(只读)。过滤 日期区间/关键词/物料类别(取物料资料)/审核情况。
    public async Task<IReadOnlyList<MaterialLabelQueryDetailRow>> LabelQueryDetailAsync(
        DateTime? 起, DateTime? 止, string? keyword, string? 物料类别, string? 审核情况)
    {
        ...
        using var c = factory.Create();
        var rows = await c.QueryAsync<MaterialLabelQueryDetailRow>($@"
...
ORDER BY h.[日期] DESC, h.[电脑单号], d.[行号];",
    ...
    // 来料标签查询·汇总：按 物料编号+规格+颜色 合并,SUM(数量)/SUM(标签数)。同过滤集。
    public async Task<IReadOnlyList<MaterialLabelQuerySummaryRow>> LabelQuerySummaryAsync(
        DateTime? 起, DateTime? 止, string? keyword, string? 物料类别, string? 审核情况)
    {
        var kw = string.IsNullOrWhiteSpace(keyword) ? null : $"%{keyword.Trim()}%";
        var cat = string.IsNullOrWhiteSpace(物料类别) ? null : 物料类别.Trim();
        var 止Excl = 止?.Date.AddDays(1);
        using var c = factory.Create();
        var rows = await c.QueryAsync<MaterialLabelQuerySummaryRow>($@"
...
ORDER BY d.[物料编号], d.[规格], d.[颜色];",
    ...
    private static List<MaterialLabelOrderLineDto> ValidateAndNormalize(MaterialLabelOrderSaveDto dto)
    {
        if (dto.日期 == default) throw new ArgumentException("日期必填。");
        ValidateLength(dto.备注一, 500, "备注一");
        ValidateLength(dto.备注二, 500, "备注二");
        if (dto.明细.Count == 0) throw new ArgumentException("来料标签单至少要有一行明细。");
        var codes = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var normalized = new List<MaterialLabelOrderLineDto>(dto.明细.Count);
        foreach (var line in dto.明细)
        {
            ...
            normalized.Add(new MaterialLabelOrderLineDto
            {
                ID = line.ID,
                物料编号 = code,
                物料名称 = line.物料名称?.Trim(),
    ...
    private static async Task InsertLinesAsync(SqlConnection c, SqlTransaction tx, long headerId,
        IReadOnlyList<MaterialLabelOrderLineDto> lines)
    {
...
(@headerId,@lineNo,@物料编号,@物料名称,@规格,@颜色,@单位,@数量,@标签数,@备注);",
    ...
    private static Task<LockedHeaderRow?> GetLockedHeaderAsync(SqlConnection c, SqlTransaction tx, string documentNo)
        => c.QuerySingleOrDefaultAsync<LockedHeaderRow>(
            "SELECT [ID],[审核] FROM [来料标签单] WITH (UPDLOCK,HOLDLOCK) WHERE [电脑单号]=@documentNo",
            new { documentNo }, tx);

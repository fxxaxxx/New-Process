...
// 报废单查询·明细行：一条报废明细(无价格;双击 单号 看整单)。
public sealed class MaterialScrapQueryDetailRow
{
...
// 报废单查询·汇总行：按 生产单号+物料编号+规格+颜色 合并(原系统"按生产单号")，SUM(数量)=报废数量。
public sealed class MaterialScrapSummaryRow
{
    public string? 生产单号 { get; set; }
    public string? 款号 { get; set; }
    public string? 物料编号 { get; set; }

...
FROM [塑胶报废明细单] WHERE [单号]=@单号 ORDER BY [ID];", new { 单号 });
    ...
    public async Task<IReadOnlyList<PlasticScrapQueryDetailRow>> ScrapQueryDetailAsync(
        DateTime 起, DateTime 止, string? keyword, string? 审核情况, string? 物料类别)
    {
        ...
        using var c = factory.Create();
        var rows = await c.QueryAsync<PlasticScrapQueryDetailRow>($@"
SELECT h.[日期], d.[单号], d.[生产单号], d.[款号], h.[报废部门], h.[报废人],
       d.[物料编号], d.[物料名称], d.[颜色], d.[塑胶货号] AS 塑胶货号, cm.[共用原料编号] AS 共用物料, cm.[塑胶货号] AS 共用货号,
       d.[单位], d.[数量], d.[单价], d.[金额], d.[备注], h.[审核]
FROM [塑胶报废明细单] d
JOIN [塑胶报废单] h ON h.[单号] = d.[单号]
...
ORDER BY h.[日期] DESC, d.[单号], d.[ID]", new { qi, qe, kw, cat });
    ...
    public async Task<IReadOnlyList<PlasticScrapQuerySummaryRow>> ScrapQuerySummaryAsync(
        DateTime 起, DateTime 止, string? keyword, string? 审核情况, string? 物料类别)
    {
        var qi = 起.Date; var qe = 止.Date.AddDays(1);
        var kw = string.IsNullOrWhiteSpace(keyword) ? null : $"%{keyword.Trim()}%";
        var cat = string.IsNullOrWhiteSpace(物料类别) ? null : 物料类别.Trim();
        using var c = factory.Create();
        var rows = await c.QueryAsync<PlasticScrapQuerySummaryRow>($@"

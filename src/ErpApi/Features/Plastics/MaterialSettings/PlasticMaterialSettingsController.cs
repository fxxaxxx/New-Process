...
[ApiController]
[Authorize]
[Route("api/plastic-material-settings")]
public sealed class PlasticMaterialSettingsController(
    PlasticMaterialSettingsService svc,
    IPermissionService perms) : ControllerBase
{
    private const string Menu = PlasticMaterialSettingsService.Menu;
    ...
    // 下游单据预填用: 任何登录用户可读(运营默认值, 非敏感), 不走设置页菜单权限,
    // 否则没有"塑胶物料设置·打开"权限的录单员拿不到预填值。
    ...
    public async Task<IActionResult> Upsert(string materialCode, [FromBody] PlasticMaterialSettingSaveDto dto)
    {

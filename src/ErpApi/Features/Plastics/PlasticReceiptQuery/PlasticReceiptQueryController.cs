...
namespace ErpApi.Features.Plastics.PlasticReceiptQuery;

[ApiController]
[Authorize]
[Route("api/plastic-receipt-query")]
public sealed class PlasticReceiptQueryController(
    PlasticReceiptService svc, IPermissionService perms) : ControllerBase
{
    private const string Menu = "塑胶入仓查询";
        ...
        [FromQuery(Name = "审核情况")] string? 审核情况 = null, [FromQuery(Name = "物料类别")] string? 物料类别 = null)
    {
        ...
        var rows = await svc.ReceiptQueryDetailAsync(起, 止, keyword, 审核情况, 物料类别);
    ...
    {
        if (!await perms.HasAsync(CurrentUser, Menu, PermissionAction.打开)) return Forbid();
        var rows = await svc.ReceiptQuerySummaryAsync(起, 止, keyword, 审核情况, 物料类别);

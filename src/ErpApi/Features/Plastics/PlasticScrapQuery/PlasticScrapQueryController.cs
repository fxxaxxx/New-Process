...
namespace ErpApi.Features.Plastics.PlasticScrapQuery;

[ApiController]
[Authorize]
[Route("api/plastic-scrap-query")]
public sealed class PlasticScrapQueryController(
    PlasticScrapService svc, IPermissionService perms) : ControllerBase
{
    private const string Menu = "塑胶报废查询";
        ...
        [FromQuery(Name = "审核情况")] string? 审核情况 = null, [FromQuery(Name = "物料类别")] string? 物料类别 = null)
    {
        ...
        var rows = await svc.ScrapQueryDetailAsync(起, 止, keyword, 审核情况, 物料类别);
        ...
        [FromQuery(Name = "审核情况")] string? 审核情况 = null, [FromQuery(Name = "物料类别")] string? 物料类别 = null)
    {
        (起, 止) = QueryDateDefaults.Normalize(起, 止);
        if (!await perms.HasAsync(CurrentUser, Menu, PermissionAction.打开)) return Forbid();
        var rows = await svc.ScrapQuerySummaryAsync(起, 止, keyword, 审核情况, 物料类别);
        if (!await CanPrice()) foreach (var r in rows) { r.单价 = null; r.金额 = null; }
        return Ok(rows);
    }
}

...
// 半成品退库查询：对 半成品退库单/明细单 的汇总与明细报表。明细自带产品列。日期为 date 型。
public sealed class SemiStockReturnQueryService(ISqlConnectionFactory factory)
{
    ...
    private (DateTime start, DateTime end, string? kw, string? match, string cmp) Common(SemiStockReturnQueryDto q)
    {
    ...
    public async Task<IReadOnlyList<SemiStockReturnSummaryRow>> SummaryAsync(SemiStockReturnQueryDto q)
    {
...
ORDER BY MAX(d.[货号]), d.[物料编号];";
        ...
        return (await c.QueryAsync<SemiStockReturnSummaryRow>(sql, new { start, end, kw, match })).AsList();
    }

    public async Task<IReadOnlyList<SemiStockReturnDetailRow>> DetailAsync(SemiStockReturnQueryDto q)
    {
        var (start, end, kw, match, cmp) = Common(q);
        var field = FieldExpr(q.Field);
        var audit = q.审核 is "0" or "1" ? q.审核 : null;
        var sql = $@"
SELECT h.[日期], h.[单号] AS [单号], h.[仓库], h.[部门] AS [退料部门], h.[退料人],
...
ORDER BY h.[日期], h.[单号], d.[ID];";
        using var c = factory.Create();
        return (await c.QueryAsync<SemiStockReturnDetailRow>(sql, new { start, end, kw, match, audit })).AsList();
    }
}

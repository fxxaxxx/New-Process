...
using ErpApi.Features.Materials.LabelOrders;
...
[Collection("db")]
public sealed class MaterialLabelOrderServiceDbTests(DbFixture fx)
{
    ...
    private MaterialLabelOrderService Svc(IAuditLogger? audit = null)
        => new(Factory(), new DocumentNumberGenerator(), audit ?? new RecordingAuditLogger());

    private static MaterialLabelOrderSaveDto ValidDto() => new()
    {
        ...
        明细 =
        [
            new() { 物料编号 = "LLB-MAT-1", 物料名称 = "物料一", 规格 = "S1", 颜色 = "红", 单位 = "个", 数量 = 7, 标签数 = 3 },
            new() { 物料编号 = "LLB-MAT-2", 物料名称 = "物料二", 规格 = "S2", 颜色 = "蓝", 单位 = "箱", 数量 = 4, 标签数 = 4 }
        ]
    };
...
SELECT CASE WHEN OBJECT_ID(N'[来料标签单]', N'U') IS NOT NULL
                  AND OBJECT_ID(N'[来料标签明细]', N'U') IS NOT NULL
                  AND OBJECT_ID(N'[单号流水表]', N'U') IS NOT NULL
             THEN 1 ELSE 0 END") == 1;
        if (!available)
        {
            c.Dispose();
            Skip.If(true, "ERP_TEST_DB 未应用来料标签单迁移(52号脚本)");
        }
        return c;
    }
    ...
    {
        var available = c.ExecuteScalar<int>(
            "SELECT CASE WHEN OBJECT_ID(N'[物料资料]', N'U') IS NOT NULL THEN 1 ELSE 0 END") == 1;
        Skip.IfNot(available, "ERP_TEST_DB 未应用物料资料迁移");
    }

    private static void CleanupCore(SqlConnection c)
    {
        c.Execute("DELETE FROM [来料标签单] WHERE [电脑单号] LIKE N'LLB%'");
        c.Execute("DELETE FROM [单号流水表] WHERE [单据类型]=N'来料标签单' AND [业务日期]='20260714'");
    }

    private static void CleanupMaterials(SqlConnection c)
    {
        c.Execute("DELETE FROM [物料资料] WHERE [物料编号] IN (N'LLB-MAT-1',N'LLB-MAT-2',N'LLB-MAT-3')");
    }

    private static void SeedMaterials(SqlConnection c)
    {
        c.Execute(@"
INSERT INTO [物料资料]([物料编号],[物料名称],[规格],[颜色],[单位],[单价]) VALUES
    (N'LLB-MAT-1',N'物料一',N'S1',N'红',N'个',12.5),
    (N'LLB-MAT-2',N'物料二',N'S2',N'蓝',N'箱',8.5),
    (N'LLB-MAT-3',N'物料三',N'S3',N'绿',N'卷',NULL);");
    }
    ...
    public async Task Create_rejects_null_line_before_opening_database()
    {
        var dto = ValidDto();
        dto.明细 = [null!];
        var service = new MaterialLabelOrderService(
    ...
    public async Task Save_load_update_and_delete_unapproved_order()
    {
        ...
        try
        {
            ...
            Assert.StartsWith("LLB", saved.电脑单号);
            ...
            Assert.False(await Svc().DeleteAsync("LLB-MISSING", "tester"));
        }
    ...
    public async Task Audit_locks_order_until_reverse_audit_and_adjacent_uses_date_then_id()
    {
        ...
        try
        {
            ...
            var invalid = new MaterialLabelOrderSaveDto { 日期 = default, 明细 = [null] };
    ...
    public async Task Materials_support_exact_and_fuzzy_filters_pagination_and_price_redaction()
    {
        ...
        try
        {
            var exact = await Svc().MaterialsAsync(new() { Field = "物料编号", Keyword = "LLB-MAT-1", Exact = true, Page = 1, Size = 20 }, true);
    ...
    public async Task Audit_failure_rolls_back_every_business_write()
    {
        ...
        try
        {
            ...
            Assert.Equal(0, c.ExecuteScalar<int>("SELECT COUNT(*) FROM [来料标签单] WHERE [电脑单号] LIKE N'LLB%'"));
    ...
    public async Task LabelQuery_detail_and_summary_filter_and_aggregate()
    {
        ...
        CleanupMaterials(c);
        c.Execute(@"
INSERT INTO [物料资料]([物料编号],[物料名称],[物料类别],[规格],[颜色],[单位]) VALUES
    (N'LLB-MAT-1',N'物料一',N'布料',N'S1',N'红',N'个'),
    (N'LLB-MAT-2',N'物料二',N'辅料资料',N'S2',N'蓝',N'箱');");
        try
        {
            var first = await Svc().CreateAsync(ValidDto(), "tester");
            var later = ValidDto();
            ...
            later.明细 =
            [
                new() { 物料编号 = "LLB-MAT-1", 物料名称 = "物料一", 规格 = "S1", 颜色 = "红", 单位 = "个", 数量 = 3, 标签数 = 2 }
            ];
            var second = await Svc().CreateAsync(later, "tester");
            ...
            // 物料类别(取物料资料)过滤。
            var byCat = await Svc().LabelQueryDetailAsync(null, null, null, "辅料资料", null);
            Assert.Single(byCat);
            Assert.Equal("LLB-MAT-2", byCat[0].物料编号);
            ...
            var m1 = summary.Single(r => r.物料编号 == "LLB-MAT-1");
            Assert.Equal(10m, m1.数量);
            Assert.Equal(5, m1.标签数);
            Assert.Equal("布料", m1.物料类别);
